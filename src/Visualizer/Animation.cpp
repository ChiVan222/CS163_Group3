...
Ani_LinkedListTraversal::Ani_LinkedListTraversal(float duration,SinglyLinkedList::Node* cur,Vector2 position, int radius) : Animations(duration),position(position),radius(radius),cur(cur)
{
        Nodes.clear(); 
        Nodes.push_back(SinglyNode(cur,position, radius)); 
}
Ani_LinkedListTraversal::Ani_LinkedListTraversal():Animations(0)
{
